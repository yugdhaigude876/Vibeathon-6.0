...
export const useStaffStore = create<StaffStoreState>((set) => ({
  ...
  addRequestToTable: (tableId: string, request: string) =>
    set((state: StaffStoreState) => ({
      tables: state.tables.map((tbl: WaiterTable) => (tbl.id === tableId ? { ...tbl, customerRequests: Array.from(new Set([...tbl.customerRequests, request])) } : tbl)),
    })),
      ...
      return {
        ...
        tables: state.tables.map((tbl) =>
          tbl.tableNumber === req.tableNumber
            ? { ...tbl, customerRequests: Array.from(new Set([...tbl.customerRequests, req.type])) }
            : tbl
        ),
      }
    }),
